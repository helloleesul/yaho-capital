        ...
        <b-container class="w-50">
            <b-row class="yellow-wrap">
                <p class="text-30 fw-900 mb-4 position-relative">
                    내 정보수정
                    <b-btn @click="changePwdToggle()" pill block :style="{background:'#ddd',border:'3px solid #000',float:'right'}">
                        <span class="fw-900 text-20 px-4" :style="{color:'#000'}">비밀번호 재설정 <font-awesome-icon :icon="changePwdShow ? 'minus' : 'plus'" /></span>
                    </b-btn>
                </p>
                <validation-observer ref="observer" v-slot="{ handleSubmit }">
                    <b-form @submit.prevent="handleSubmit(updateMe)">
                        ...
                        <b-button type="submit" class="mt-4" v-if="update" pill block :style="{background:'#FDDA00',border:'3px solid #000'}">
                            <span class="text-20 px-4 fw-900" :style="{color:'#000'}">정보수정</span>
                        </b-button>
                    </b-form>
                </validation-observer>
            </b-row>
