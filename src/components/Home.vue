<template>
  <main>
      ...
      <section class="border-bottom">
        <b-container >
          <b-row class="text-center align-items-center">
            <p class="fw-900 text-30 mb-3">필요할 때만 쓰고, 이자는 사용한 만큼만</p>
            <span class="text-20" :style="{color:'#aaa'}">
              매일 사용한 금액에 대해서만 이자가 발생합니다. <br/>
              필요할 때만 꺼내쓰고, 언제든지 편리하게 돈을 채워넣어 이자의 부담을 덜 수 있습니다.
            </span>
          </b-row>
        </b-container>
      </section>
        ...
        <b-container>
          <b-row class="m-auto">
            <b-col>
              ...
              <span class="d-block text-18">방문없이 인터넷, 스마트폰, 전화로 <br/>대출신청이 가능한 상품입니다.</span>
            </b-col>
            <b-col class="col-7">
              <table>
                <thead>
                  <tr>
                    <th>구분</th>
                    <th :style="{background:'#535D71', color:'#fff'}">내용</th>
                  </tr>
                </thead>
                <tbody>
        ...
        <b-container>
          <b-row class="m-auto">
            <b-col>
              ...
              <span class="d-block text-18">담당 상담원이 말씀드린 서류를 <br/>안내한 FAX번호로 보내주세요.</span>
            </b-col>
            <b-col class="col-7">
              <ol class="p-0 m-0">
          ...
          <b-row class="text-center align-items-center justify-content-center">
            <div>
              ...
              :style="{flexWrap:'wrap', gap:'10px'}">
                <li v-for="item in loanItems" :key="item.index" :style="{width:'16%'}">
                  <b-btn @click="modalShow(item)" class="w-100 position-relative border-0" :style="{ height:'8rem', background: item.color }">
                    <span class="text-20 loanItem">
                      {{item.name}}
                    </span>
                    <font-awesome-icon class="loanItem-icon fa-3x" :icon="item.icon" />
                  </b-btn>
                </li>
              </ul>
            </div>
  ...
  data() {
    return {
      ...
      loanItems: [
        ...
        {name: '저신용자희망대출', color: '#526BAA', icon: 'file-invoice-dollar'},
      ],
      steps: [
        {title:'대출상담 신청하기', content:'대출상품을 클릭 후 내용을 기입하시고 대출신청완료 버튼을 눌러주세요.'},
        {title:'담당 상담원과 통화', content:'신청하신 연락처로 담당 상담원이 대출상품 및 필요한 서류들을 안내해드립니다.'},
        {title:'구비서류 준비', content:'담당 상담원이 말씀드린 서류를 안내한 FAX번호로 보내주세요.'},
        {title:'대출승인', content:'대출심사완료 후 고객님이 기재하신 연락처로 대출승인금액을 통보해드립니다.'},
        {title:'대출금 송금', content:'대출 승인 완료 후 고객님의 온라인 계좌로 송금해드립니다.'},
      ],
      table: [
